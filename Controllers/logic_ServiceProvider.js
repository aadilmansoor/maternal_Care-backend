...
exports.serviceProviderRegistration = async(req,res)=>{
    ...
    try{
        ...
        if(existingUser && registereduser){
            ...
            const newUser = new servicerproviders({username,email,password,mobile,profile_image:"",service,specialization,qualification,experience_crt:exp_crt,exp_year,rate,status:"pending"})
           await newUser.save()
           res.status(200).json({newUser, message:'primary registration completed,'})
...
exports.serviceProviderfinalRegistration=async(req,res)=>{
    ...
    try{
        ...
        if(!existingUser){
            ...
            {
                const preUser = await servicerproviders.findOne({email})
                res.status(200).json({preUser, message:"photo successfully updated"})
            }
            else{
...
exports.loginServiceProvider = async (req,res)=>{
    ...
    try{
       const existingUser = await approvedservicerproviders.findOne({email,password})
       if(existingUser!==null && existingUser!==undefined){
        ...
        res.status(200).json({existingUser,token,message:"login Successfully"})
       }
